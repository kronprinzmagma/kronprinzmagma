...
Five public repos. The business problem comes first in every README — the engineering proof (architecture, tests, evals, cost-tracking) is one click away inside each repo.

Four of them are sanitised mirrors of a private working repo, kept in sync after every release: the code is identical, only real personal data is replaced with synthetic test sets. `ki-news-aggregator` is the exception — it is developed in the open, because it never touches personal data in the first place.
